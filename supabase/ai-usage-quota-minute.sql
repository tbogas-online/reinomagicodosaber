-- Pressão de quota tokens/min (cabeçalhos API + eventos de limite).
-- Executar no SQL Editor do Supabase após ai-usage-latency-buckets.sql.

ALTER TABLE public.ai_request_minute_dims
  ADD COLUMN IF NOT EXISTS limit_count INT NOT NULL DEFAULT 0 CHECK (limit_count >= 0),
  ADD COLUMN IF NOT EXISTS quota_used_peak INT NOT NULL DEFAULT 0 CHECK (quota_used_peak >= 0);
...
BEGIN
  ...
  END;
  v_limit_hit := CASE WHEN COALESCE(p_limit_hit, false) THEN 1 ELSE 0 END;
...
CREATE OR REPLACE FUNCTION public.record_ai_quota_minute_pressure(
  p_provider TEXT DEFAULT '',
  p_quota_tokens_used INT DEFAULT NULL,
  p_limit_hit BOOLEAN DEFAULT false
)
...
BEGIN
  ...
  v_quota_used := GREATEST(COALESCE(p_quota_tokens_used, 0), 0);
  v_limit_hit := CASE WHEN COALESCE(p_limit_hit, false) THEN 1 ELSE 0 END;
  IF v_provider = '' OR (v_quota_used <= 0 AND v_limit_hit = 0) THEN
    RETURN;
  END IF;
  ...
  VALUES (
    v_minute_bucket, v_provider, '__quota__', 0, 0, 0, 0, 0, v_limit_hit, v_quota_used
  )
  ON CONFLICT (bucket_start, provider, model) DO UPDATE SET
    limit_count = m.limit_count + v_limit_hit,
    quota_used_peak = GREATEST(m.quota_used_peak, v_quota_used);
...
GRANT EXECUTE ON FUNCTION public.increment_ai_request_bucket(BOOLEAN, TEXT, TEXT, INT, INT, BOOLEAN, INT) TO service_role;
GRANT EXECUTE ON FUNCTION public.record_ai_quota_minute_pressure(TEXT, INT, BOOLEAN) TO service_role;

DROP FUNCTION IF EXISTS public.increment_ai_request_bucket(BOOLEAN, TEXT, TEXT, INT, INT);
