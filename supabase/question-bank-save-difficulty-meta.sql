-- Grava dificuldade pedida + estimada ao guardar no banco.
-- Executar no SQL Editor do Supabase após question-bank-difficulty.sql
-- e question-bank-difficulty-by-age.sql.

ALTER TABLE public.question_bank
  ADD COLUMN IF NOT EXISTS estimated_difficulty SMALLINT;

ALTER TABLE public.question_bank
  DROP CONSTRAINT IF EXISTS question_bank_estimated_difficulty_check;

ALTER TABLE public.question_bank
  ADD CONSTRAINT question_bank_estimated_difficulty_check
  CHECK (estimated_difficulty IS NULL OR (estimated_difficulty >= 1 AND estimated_difficulty <= 5));

DROP FUNCTION IF EXISTS public.save_question_to_bank(INT, TEXT, TEXT, TEXT, TEXT, JSONB, TEXT, TEXT, TEXT, TEXT, TEXT, NUMERIC, SMALLINT);
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF p_difficulty IS NOT NULL AND v_existing.difficulty IS NULL THEN
      ...
      WHERE question_hash = p_question_hash;
    END IF;
    IF p_estimated_difficulty IS NOT NULL AND v_existing.estimated_difficulty IS NULL THEN
      UPDATE public.question_bank
      SET estimated_difficulty = p_estimated_difficulty
      WHERE question_hash = p_question_hash;
    END IF;
    IF p_difficulty IS NOT NULL
       AND p_age_band IN ('6-9', '10-15', '15+')
       AND (v_existing.difficulty_by_age_band IS NULL
            OR v_existing.difficulty_by_age_band ->> p_age_band IS NULL) THEN
      v_by_age := COALESCE(v_existing.difficulty_by_age_band, '{}'::jsonb)
        || jsonb_build_object(p_age_band, p_difficulty);
      UPDATE public.question_bank
      SET difficulty_by_age_band = v_by_age
      WHERE question_hash = p_question_hash;
    END IF;
  ...
  v_by_age := NULL;
  IF p_difficulty IS NOT NULL AND p_age_band IN ('6-9', '10-15', '15+') THEN
    v_by_age := jsonb_build_object(p_age_band, p_difficulty);
  END IF;

  INSERT INTO public.question_bank (
    category_n, age_band, question, correct_answer,
    options, format, knowledge_key, question_hash, source,
    knowledge_id, source_id, confidence, difficulty, estimated_difficulty,
    difficulty_by_age_band
  ) VALUES (
    ...
    p_difficulty, p_estimated_difficulty, v_by_age
  );
...
GRANT EXECUTE ON FUNCTION public.save_question_to_bank(
  INT, TEXT, TEXT, TEXT, TEXT, JSONB, TEXT, TEXT, TEXT, TEXT, TEXT, NUMERIC, SMALLINT, SMALLINT
