-- Contadores agregados de pedidos à IA (buckets de 5 minutos, retenção 48 h).
-- Buckets de 1 minuto (última hora) + tokens por pedido (retenção 2 h).
...
CREATE INDEX IF NOT EXISTS idx_ai_request_buckets_start
  ON public.ai_request_buckets (bucket_start DESC);
