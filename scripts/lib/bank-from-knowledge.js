...
const { buildAdivinhaDistractors } = require('./adivinha-distractors-node');
const {
  loadQuestionEngine,
  stripTags,
  normalizeQ,
} = require('./adivinha-bank-audit');
...
function buildAdivinhaQuestion(record, QE) {
  ...
  const distractors = buildAdivinhaDistractors(answer, { normalizeFn: normalizeQ })
    || QE.buildAdivinhaDistractors?.(answer, { normalizeFn: normalizeQ });
  if (!distractors) return null;
  const options = QE.assembleMcOptions(answer, distractors);
...
function buildBankItems(records, ageBand, QE, { source = 'repository-replenish' } = {}) {
  const items = [];
  let skipped = 0;
  for (const record of records) {
    const built = buildAdivinhaQuestion(record, QE);
    if (!built) {
      skipped += 1;
      continue;
    }
...
async function replenishBankFromKnowledge(url, key, {
  ...
  const { items, skipped } = buildBankItems(records, ageBand, engine, { source });
  if (dryRun) {
    return {
      ok: true,
      dryRun: true,
